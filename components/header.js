    ...
    return (
      <header className={scroll || path!="/" ? "fix" : ""}>
        <div className="contain-fluid">
          <div className="logo">
              <Link href="/">
                  <Image src={cmsFileUrl(siteSettings.site_settings.site_logo)} width={180} height={80} alt={siteSettings.site_settings.site_name} />
              </Link>
          </div>
          ...
          <nav id="nav" className={toggle ? "active" : ""}>
            <ul>
              ...
              <li className="drop">
                <Link href="/" onClick={ToggleFeatures}>Features</Link>
                <ul className={features ? "sub active" : "sub"}>

                  {siteSettings.features.map((feature) => {

                    return (
                        <li key={feature.id}><Link href={`/features/${feature.slug}`}><span><Image src={cmsFileUrl(feature.icon, 'features')} width={50} height={50} alt={feature.name} /></span><span><Text string={feature.name} /><em><Text string={feature.short_desc} /></em></span></Link></li>
              ...
              <li className="drop">
                <Link href="/" onClick={ToggleSolutions}>Solutions</Link>
                <ul className={solutions ? "sub active" : "sub"}>

                  {siteSettings.solutions.map((sol) => {
                    return (
                      <li key={sol.id}><Link href={`/solutions/${sol.slug}`}><span><Image src={cmsFileUrl(sol.icon, 'solutions')} width={50} height={50} alt={sol.title} /></span><span><Text string={sol.title} /><em><Text string={sol.short_desc} /></em></span></Link></li>
              ...
              <li className="hide_navigation_lnk_desktop"><Link href="/contact">Contact Us</Link></li>
            </ul>
          </nav>
          <div className="login_btns">
            <ul>
              <li className="hide_navigation_lnk_mobile"><Link href="/contact">Contact Us</Link></li>
              <li className="btn_blk"><Link href="/login" className="site_btn">Login</Link></li>
            </ul>
          </div>
